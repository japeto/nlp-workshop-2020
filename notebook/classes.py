...
class Embedding:
    """
    This class represents one word embedding as word and the vector it is assigned.
    """
    ...
    def __sub__(self, embedding):
        """
        Calculates substraction of self and given embedding as new Embedding.
        :param embedding: Embedding
        :return: Embedding
        """
        if len(self) != len(embedding):
            raise KeyError('The embeddings have different lengths')
        vector = [self.v[i] - embedding.v[i] for i in range(len(self))]
...
class Pair:
    ...
    def cosine_similarity(self, pair):
        """
        Cosine similarity between self and some other pair.
        :param pair: Pair
        :return: float
        """
        return utils.cosine_similarity(self.pair_embedding, pair.pair_embedding)

    def euclidean_similarity(self, pair):
        """
        Euclidean similarity between self and some other pair.
        :param pair: Pair
        :return: float
        """
        return utils.euclidean_similarity(self.pair_embedding, pair.pair_embedding)
    ...
    def svm_values(self, pairs, distance='euclidean'):
        """
...
class PairSet:
            ...
            for pair in pairs:
                if pair[0] in model and pair[1] in model:
                    seed_pairs.append(Pair(
                        embedding_object(pair[0]),
                        embedding_object(pair[1]),
                        positive=True))
                else:
                    print("%s or %s is not in word embeddings model." % (pair[0], pair[1]))
    ...
    def spatial_candidates_words(self, size=100, filter_positives=True):
        ...
        for rel in self.set_pairs:
            ng_1 = [record[0] for record in self.model.most_similar(rel.e_1.word, topn=size)]
            ng_2 = [record[0] for record in self.model.most_similar(rel.e_2.word, topn=size)]
    ...
    def comparative_algorithm(self,
        ...
        if weight_type == 'none':
            weights = [float(1) / len(seed) for _ in range(len(seed))]
        elif weight_type == 'standard':
            weights = list_normalize([seed.pair_weight(pair, distance) for pair in seed.set_pairs])
        elif weight_type == 'softmax':
            weights = list_softmax([seed.pair_weight(pair, distance) for pair in seed.set_pairs])
        else:
    ...
    def svm_learning(self,seed=None, testing=None, candidates=None,
                     distance='euclidean', neighborhood=100,
                     svm_folder = None, svm_perf_path=None,
                     **kwargs):
        """
        Calculates SVM Learning algorithms from out method on given data. If seed is not set, it is calculated over
        ...
        print(f"Start SVM [svm_perf_learn] {timestamp}")
        # name files
        if svm_folder:
            train_filename = svm.svm_file_name(name, timestamp, 'train', svm_folder=svm_folder)
            model_filename = svm.svm_file_name(name, timestamp, 'model', svm_folder=svm_folder)
            test_filename = svm.svm_file_name(name, timestamp, 'test', svm_folder=svm_folder)
            prediction_filename = svm.svm_file_name(name, timestamp, 'prediction', svm_folder=svm_folder)
        else:
        ...
        # print(f"build trainig file in: {train_filename}")
        # # open(train_filename, "wb").write(
        # #     bytes('\n'.join([rel.svm_transform(examples, distance) for rel in positive + candidates]), "utf-8"))
        # #
        # # print(f"build test file in: {test_filename}")
        # # open(test_filename, "wb").write(
        # #     bytes('\n'.join([rel.svm_transform(examples, distance) for rel in evaluated]), "utf-8"))
        #
        # if svm_perf_path:
        #     learn_path = f'{svm_perf_path}/svm_perf_learn -v 0 -l 10 -c 0.01 -w 3 ' \
        #                  f'{train_filename} {model_filename}'
        #     print(f"learn... {learn_path}")
        #     # os.system(learn_path)
        #
        #     classify_path = f'{svm_perf_path}/svm_perf_classify -v 0 ' \
        #                     f'{test_filename} {model_filename} {prediction_filename}'
        #
        #     print(f"classify... {classify_path}")
        #     # os.system(classify_path)
        # else:
        #     print(f"learn . . . ")
        #     os.system(f'./lib/svm_perf/svm_perf_learn -v 0 -l 10 -c 0.01 -w 3 {train_filename} {model_filename}')
        #     print(f"classify . . .")
        #     os.system(f'./lib/svm_perf/svm_perf_classify -v 0 {test_filename} {model_filename} {prediction_filename}')

        print(f"load predictions from {prediction_filename}")
        tagged = svm.load_results(test_filename, prediction_filename)
        return tagged
    ...
    def find_new_pairs(self, result_count=100, output="", method='avg', **kwargs):
        """
        ...
        if method == 'svm':
             results = self.svm_learning(method=method, **kwargs)
        return results

        #if results:
        #    results.print_top_n_to_file(result_count, output)
        #else:
        #    raise KeyError('method not setting')
    ...
    def positions_testing(self, repeat=10, method='avg', **kwargs):
        """
        ...
        with value 'avg', 'max' or 'svm'. Details about other attributes in kwargs
        :param repeat: integer
        :param mmethod: 'avg', 'svm' or 'max'
        :param kwargs: algorithm parameters
        :return: None
        """
        positions = []
        ...
        for _ in range(repeat):
            ...
            if method == 'max' or method == 'avg':
                results = self.comparative_algorithm(seed=training, testing=testing, candidates=candidates,
                                                     method=method, **kwargs)
            if method == 'svm':
                results = self.svm_learning(seed=training, testing=testing, candidates=candidates, method=method,
                                           **kwargs)
...
class Result:
    ...
    def __repr__(self):
        ...
        """
        return {
            "name":self.name, "is_positive":self.is_positive,
            "score":self.score,"position":self.position
        }
    ...
    def __str__(self):
        ...
        :return:
        """
        return f"{self.is_positive} {self.name} {self.score}"
...
class ResultList:
    ...
    def from_array(self, array=None):
        """
        Add elements from array to ResultList
        :param array:
        :return:
        """
        for each in array:
            self.append(is_positive=each["is_positive"], name=each["name"],
                        score=each["score"])

    def print(self, lines=100, in_color=True):
        """
        ...
        for i in range(lines):
            if in_color:
                line = self[i].name.split("-")
                if self[i].is_positive:
                    print(f"{colored(self[i].is_positive, 'red')} " +
                          f"{colored(line[0], 'green')} {colored(line[1], 'blue')}")
                else:
                    print(f"{colored('?', 'red')} {colored(line[0], 'green')} {colored(line[1], 'blue')}")
            else:
