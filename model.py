"""Model
This script load w2vec model, and find new candidates
"""
...
import model_utils as svm

#Creates global variable 'model' used in all classes.
# TODO: improve POO, with a singlenton instance
...
class PairSet:
    ...
    def svm_learning(self,
        ...
        return svm.load_results(timestamp)


    def find_new_pairs(self, result_count=100, output=config.output_file, method='avg', **kwargs):
        """
        Finds new pairs from given set and prints them to file. result_count is number of results printed to file
        with a name set by parameter output. method is name of rating method: 'max', 'avg' or 'svm'.
    ...
    def positions_testing(self, repeat=10, method='avg', **kwargs):
        ...
        precise are the results. However this experiment is very timely.
        :param repeat: integer
        :param mmethod: 'avg', 'pu' or 'max'
        :param kwargs: algorithm parameters
        :return: None
        """
        positions = []
        ...
        for _ in range(repeat):
            ...
            if method == 'max' or method == 'avg':
                results = self.comparative_algorithm(seed=training, testing=testing, candidates=candidates, method=method, **kwargs)
            if method == 'svm':
                results = self.svm_learning(seed=training, testing=testing, candidates=candidates, method=method, **kwargs)
